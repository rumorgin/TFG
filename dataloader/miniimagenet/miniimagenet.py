...
class MiniImageNet(Dataset):
    ...
    def __init__(self, root='./data', train=True,
        ...
        self.WORD_EMEDDING_PATH = os.path.join(root, 'miniimagenet/word_ebedding')
        ...
        word2vec_csv=osp.join(self.WORD_EMEDDING_PATH, 'bert_embeddings_imagenet.csv')

        with open(word2vec_csv, 'r') as csv_file:
            csv_reader = csv.reader(csv_file)
            header = next(csv_reader)  # Read the header
            self.word_embedding = {row[0]: row[2] for row in csv_reader}
    ...
    def __getitem__(self, i):
        ...
        word_embedding= torch.tensor(ast.literal_eval(self.word_embedding[self.wnids[targets]]))
        image = self.transform(Image.open(path).convert('RGB'))
        return image, targets, word_embedding
        # return image, targets


if __name__ == '__main__':
    txt_path = r"D:\fscil_lmu\data\index_list\mini_imagenet\session_1.txt"
    # class_index = open(txt_path).read().splitlines()
    base_class = 100
    class_index = np.arange(base_class)
    dataroot = r'D:\fscil_lmu\data'
    batch_size_base = 400
    ...
    for batch in trainloader:
        images, numeric_labels, bert_embeddings = batch
